#!/usr/bin/env python3
"""Feather/Wings-style TTY stop simulator for Multi Minecraft.

Runs the container entrypoint inside a real pty (exactly how Wings-family
daemons create Tty:true containers), waits for the server to report ready
in the console mirror, then writes the literal stop command text (^C) into
the pty master - the same bytes a daemon sends through its attach stream.
Propagates the entrypoint's exit code.
"""
import os
import pty
import select
...
READY = os.environ.get("TTY_READY_MATCH", "TEST-SERVER-UP").encode()
...
ready = False
# The entrypoint mirrors the whole console (incl. server output) into the
# shared log; poll it for the app's ready marker.
